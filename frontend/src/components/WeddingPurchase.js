...
import { checkWeddingSlug, createWeddingCheckout } from '../api/api';
...
const WeddingPurchase = () => {
  const searchParams = useSearchParams();
  const themeId = searchParams.get('theme_id');

  const [form, setForm] = useState({ couple_name: '', wedding_date: '', slug: '', host_email: '', host_name: '', password: '', confirm_password: '' });
  ...
  const handleSlugBlur = async () => {
    ...
    try {
      const res = await checkWeddingSlug(slug);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await createWeddingCheckout(payload);
    ...
    <div>
      <section className="hero">
        <div className="container">
          <h1>💍 Create Your Wedding Page</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-[50px] max-w-[900px] mx-auto">
            <form onSubmit={handleSubmit} className="bg-white p-5 md:p-10 rounded-[12px] shadow-[0_2px_15px_rgba(0,0,0,0.08)]">
              {isExistingHost ? (
                <div className="py-3 px-4 rounded-[8px] mb-5 bg-[#fff0f7] border border-[#ff6b9d] text-[#333] text-[0.9rem]">
                  You're adding a new wedding to your existing account.{' '}
              ...
              <div className="flex flex-col gap-2 mb-5">
                <label htmlFor="couple_name" className="font-semibold text-black text-[0.95rem]">Couple's Names *</label>
                <input type="text" id="couple_name" name="couple_name" value={form.couple_name} onChange={handleChange} required placeholder="e.g. Sarah & James" className={inputClass} />
              </div>

              <div className="flex flex-col gap-2 mb-5">
                <label htmlFor="wedding_date" className="font-semibold text-black text-[0.95rem]">Wedding Date *</label>
                <input type="date" id="wedding_date" name="wedding_date" value={form.wedding_date} onChange={handleChange} min={new Date().toISOString().split('T')[0]} required className={inputClass} />
              ...
              <div className="flex flex-col gap-2 mb-5">
                ...
                <small className="text-[#666] text-[0.85rem]">Only letters, numbers, and hyphens. Example: sarah-and-james</small>
              </div>
              ...
              <div className="flex flex-col gap-2 mb-5">
                ...
                <small className="text-[#666] text-[0.85rem]">Your receipt and wedding page link will be sent here.</small>
              </div>
              ...
              {!isExistingHost && (
                <>
                  <div className="flex flex-col gap-2 mb-5">
                    ...
                    <small className="text-[#666] text-[0.85rem]">You'll use this to log into your wedding dashboard later.</small>
                  </div>
            ...
            <div className="bg-[#fff8ff] border-2 border-[#ff6b9d] rounded-[12px] p-[30px] h-fit sticky top-5 md:static">
              ...
              <ul className="list-none p-0 mb-[25px]">
                {['Custom URL for your wedding page', 'Photo gallery for guests', 'Guest book messages', 'RSVP tracking', 'Our Story timeline', 'Live countdown to the wedding', 'Active for 60 days after the event'].map(item => (
              ...
              <div className="flex justify-between items-center mt-5 pt-5 border-t-2 border-[#ff6b9d]">
                ...
                <strong className="text-[1.8rem] text-[#ff6b9d]">$49</strong>
              </div>
            </div>
          </div>
        </div>
...
export default WeddingPurchase;
