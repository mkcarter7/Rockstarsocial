...
def delete_photo(request, slug, photo_id):
    ...
    if delete_token:
        try:
            photo = WeddingPhoto.objects.get(id=photo_id, event__slug=slug, delete_token=delete_token)
            photo.delete()
            return Response({'message': 'Photo deleted'})
        except WeddingPhoto.DoesNotExist:
    ...
    if session_token:
        ...
        if not err:
            try:
                photo = WeddingPhoto.objects.get(id=photo_id, event=event)
                photo.delete()
                return Response({'message': 'Photo deleted'})
            except WeddingPhoto.DoesNotExist:
    ...
    if get_firebase_user(request):
        try:
            photo = WeddingPhoto.objects.get(id=photo_id, event__slug=slug)
            photo.delete()
            return Response({'message': 'Photo deleted'})
        except WeddingPhoto.DoesNotExist:
            return Response({'error': 'Photo not found'}, status=status.HTTP_404_NOT_FOUND)
