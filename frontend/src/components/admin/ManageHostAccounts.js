...
import { getAdminHostAccounts, adminResetHostPassword, adminDeleteHostAccount, adminSendMagicLink, adminSendMagicLinkByEmail } from '../../api/api';
...
const ManageHostAccounts = () => {
  ...
  const handleSendLink = async (accountId, email, isOrphaned) => {
    const key = isOrphaned ? email : accountId;
    setLinkStatus({ id: key, type: 'loading', message: 'Sending...' });
    try {
      const token = await getIdToken();
      if (isOrphaned) {
        await adminSendMagicLinkByEmail(email, token);
      } else {
        await adminSendMagicLink(accountId, token);
      }
      setLinkStatus({ id: key, type: 'success', message: 'Link sent!' });
    } catch (err) {
      const msg = err?.response?.data?.error || 'Failed to send link.';
      setLinkStatus({ id: key, type: 'error', message: msg });
    }
  };
      ...
      <div className="py-[30px] px-10 md:px-5">
        {accounts.length === 0 ? (
          ...
          <table className="w-full border-collapse bg-white rounded-[8px] overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.08)] text-[0.9rem]">
            <thead>
              <tr>
                {['Email', 'Created', 'Events', 'Event Slugs', ''].map(h => (
            ...
            <tbody>
              {accounts.map(account => {
                const rowKey = account.orphaned ? account.email : account.id;
                const linkKey = account.orphaned ? account.email : account.id;
                return (
                <React.Fragment key={rowKey}>
                  <tr style={account.orphaned ? { background: '#fffbea' } : {}}>
                    <td className="py-3 px-4 border-b border-[#eee] align-middle font-medium">
                      {account.email}
                      {account.orphaned && (
                        <span className="ml-2 text-[0.7rem] bg-[#fef3c7] text-[#92400e] border border-[#fcd34d] rounded-[4px] px-1.5 py-0.5 font-semibold uppercase tracking-wide">
                          No account
                        </span>
                      )}
                    </td>
                    <td className="py-3 px-4 border-b border-[#eee] align-middle">
                      {account.created_at ? formatDate(account.created_at) : <span className="text-[#aaa] italic text-[0.85rem]">—</span>}
                    </td>
                    <td className="py-3 px-4 border-b border-[#eee] align-middle text-center">{account.party_count}</td>
                    <td className="py-3 px-4 border-b border-[#eee] align-middle">
                      <div className="flex flex-wrap gap-2">
                        {account.parties.map(p => {
                          const typeColor = p.event_type === 'wedding' ? '#c9a96e'
                            : p.event_type === 'baby_shower' ? '#c17c5a'
                            : '#ff6b9d';
                          const typeLabel = p.event_type === 'wedding' ? 'Wedding'
                            : p.event_type === 'baby_shower' ? 'Baby Shower'
                            : 'Birthday';
                          return (
                            <div key={p.slug} className="flex flex-col gap-0.5">
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="font-mono text-[0.8rem] hover:underline"
                                style={{ color: typeColor }}
                                title={`${p.display_name} — ${p.is_active ? 'active' : 'inactive'}`}
                              >
                                /{p.slug}
                              </a>
                              <span className="text-[0.7rem] font-semibold uppercase tracking-wide" style={{ color: typeColor }}>
                                {typeLabel}
                              </span>
                            </div>
                          );
                    ...
                    <td className="py-3 px-4 border-b border-[#eee] align-middle">
                      <div className="flex gap-2 items-center flex-wrap">
                        <button
                          onClick={() => handleSendLink(account.id, account.email, account.orphaned)}
                          disabled={linkStatus.id === linkKey && linkStatus.type === 'loading'}
                        ...
                        >
                          {linkStatus.id === linkKey && linkStatus.type === 'loading' ? 'Sending...' : 'Send Login Link'}
                        </button>
                        {linkStatus.id === linkKey && linkStatus.type !== 'loading' && (
